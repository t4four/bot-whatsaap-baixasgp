...
const targetGroupId = ''; // Substitua pelo ID do grupo desejado
        ...
        if (message.body.startsWith('!baixar')) {
            ...
            if (parts.length > 1) {
                ...
                const consultOptions = {
                    method: 'POST',
                    url: 'https://suaurl/api/ura/consultacliente/',
                    headers: {},
                    formData: {
                        'app': '',
                        'token': '',
                ...
                request(consultOptions, function (error, response) {
                    ...
                    try {
                        ...
                        if (result && result.contratos && result.contratos.length > 0) {
                            ...
                            const tituloOptions = {
                                method: 'POST',
                                url: 'https://suaurl/api/ura/titulos/',
                                headers: {
                                ...
                                try {
                                    ...
                                    if (tituloResult && tituloResult.titulos && tituloResult.titulos.length > 0) {
                                        ...
                                        titulos.forEach(titulo => {
                                            const baixaOptions = {
                                                method: 'POST',
                                                url: `https://suaurl/api/banco/titulo/${titulo.id}/baixar/`,
                                                headers: {
                                                    'Content-Type': 'application/json'
                                                },
                                                body: JSON.stringify({
